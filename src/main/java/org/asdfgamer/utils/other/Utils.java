...
public class Utils {
     ...
     * Moreover it checks if the Element is in the Enum.
     * This means that "java.lang.annotation.ElementType.Hello" would return false.
     * <p>
     * If the Enum that is checked isn't fully initialised this function returns always false. You could use isEnumClass
     * to check if the class is an Enum at that time.
     *
     * @param enumObject The Object that should be checked.
     * @return true, if it ist an enum, otherwise false.
     */
    public static boolean isEnumElement(Object enumObject) {
        return isEnum(enumObject, true);
    }

    /**
     * This tests if the given object is an Enum, or an Class object of an Enum or a String that represents an Enum.
     * This means that "java.lang.annotation.ElementType.Hello" would return true.
     *
     * @param enumObject The Object that should be checked.
     * @return true, if it ist an enum, otherwise false.
     */
    public static boolean isEnumClass(Object enumObject) {
        return isEnum(enumObject, false);
    }
     ...
     *
     * @param enumObject   The Object that should be checked.
     * @param checkElement true, if the element should be checked (like in isEnumElement)
     * @return true if it is an enum.
     */
    private static boolean isEnum(Object enumObject, boolean checkElement) {
                ...
                if (((String) enumObject).contains(".")) {
                    try {
                        ...
                        if (Class.forName(shortName).isEnum()) {
                            ...
                            Object[] constants = aClass.getEnumConstants();//((Object[]) constantsObj);
     ...
     * <p>
     * NOTE: This does not work if the enum isn't fully initialised.
     *
    ...
    /**
     * This Method checks if the Stacktrace of the current Thread contains an Element of the given Class and Method.
     *
     * @param aClass The Class that should be in the Stacktrace.
     * @param method The Method of the Class that should be in the Stacktrace or null
     * @return true if the Class (and the Method) are in the Stacktrace.
     */
    public static boolean stackTraceContains(Class aClass, Method method) {
        StackTraceElement[] traceElements = Thread.currentThread().getStackTrace();
        for (StackTraceElement traceElement : traceElements) {
            if (traceElement.getClassName().equals(aClass.getName())) {
                if (method != null) {
                    if (traceElement.getMethodName().equals(method.getName())) {
                        return true;
                    }
                } else {
                    return true;
